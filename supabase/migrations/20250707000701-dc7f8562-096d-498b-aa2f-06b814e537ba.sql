...
-- Fix the handle_new_user function to resolve the ambiguous user_id reference
...
CREATE OR REPLACE FUNCTION public.initialize_default_exercises(target_user_id UUID)
RETURNS void AS $$
DECLARE
  chest_id UUID;
  back_id UUID;
...
BEGIN
  ...
  INSERT INTO public.muscle_groups (user_id, name, is_default) VALUES
    (target_user_id, 'Chest', true),
    (target_user_id, 'Back', true),
    (target_user_id, 'Legs', true),
    (target_user_id, 'Biceps', true),
    (target_user_id, 'Triceps', true),
    (target_user_id, 'Shoulders', true),
    (target_user_id, 'Abs', true);
  
  -- Get the IDs of the created muscle groups
  SELECT id INTO chest_id FROM public.muscle_groups WHERE user_id = target_user_id AND name = 'Chest';
  SELECT id INTO back_id FROM public.muscle_groups WHERE user_id = target_user_id AND name = 'Back';
  SELECT id INTO legs_id FROM public.muscle_groups WHERE user_id = target_user_id AND name = 'Legs';
  SELECT id INTO biceps_id FROM public.muscle_groups WHERE user_id = target_user_id AND name = 'Biceps';
  SELECT id INTO triceps_id FROM public.muscle_groups WHERE user_id = target_user_id AND name = 'Triceps';
  SELECT id INTO shoulders_id FROM public.muscle_groups WHERE user_id = target_user_id AND name = 'Shoulders';
  SELECT id INTO abs_id FROM public.muscle_groups WHERE user_id = target_user_id AND name = 'Abs';
